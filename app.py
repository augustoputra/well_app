...
# ================================
PSN = st.number_input("PSN", *RANGES["PSN"])
AVE_GROSS = st.number_input("Average Fluid Gross (bopd)", *RANGES["AVE_GROSS"])
AVE_GAS = st.number_input("Average GAS (Mscfd)", *RANGES["AVE_GAS"])
PUMP_EFF = st.number_input("Pump Efficiency", *RANGES["PUMP_EFF"])
OD_PUMP = st.number_input("Pump OD", *RANGES["OD_PUMP"])
SL = st.number_input("Stroke Length", *RANGES["SL"])
SPM = st.number_input("SPM", *RANGES["SPM"])
SM = st.number_input("Submergence", *RANGES["SM"])
TORQUE = st.number_input("Torque", *RANGES["TORQUE"])
LOAD = st.number_input("Load", *RANGES["LOAD"])
ROD_STRESS = st.number_input("Rod Stress", *RANGES["ROD_STRESS"])
FREQ_OFF = st.number_input("Freq Off", *RANGES["FREQ_OFF"])
HOUR_OFF = st.number_input("Hour Off", *RANGES["HOUR_OFF"])
